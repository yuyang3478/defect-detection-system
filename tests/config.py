...
tf_initial_checkpoint = "models/research/deeplab/datasets/crack/exp/init_models/xception_65/model.ckpt"
train_logdir = "models/research/deeplab/datasets/crack/exp/train"
dataset_dir = "models/research/deeplab/datasets/crack/tfrecord"
'''
...
def config_write():
    ...
    with open('config/model.ini', 'w') as configfile:
...
def config_func():
    '''
    写入的值均为字符串
    配置文件的节名称区分大小写, 键不区分大小写(可任意缩进), 注释用'#'和';'(用作整行前缀,可缩进,不推荐行内注释), 值可以跨越多行(要缩进,慎用), 键值分隔符'='和':'
